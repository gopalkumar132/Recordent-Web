                    	...
                    	<div class="table-responsive">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <?php } else {?>
                                     <td class="balance">{{General::ind_money_format($data->due_amount - General::getPaidForDueOfBusiness($data->dueId, $userId))}}</td>
                                    <?php }?>
                                    ...
                                    <td align="center">
                                		@if(!empty($data->proof_of_due))
                                				...
                                				<?php 
													$imgProof=str_replace("business/proof_of_due/","",$data->proof_of_due);
													$imgProof=trim($imgProof,",");
                                                    $imgList=explode(",",$imgProof);
                                                    foreach($imgList as $img_name){
														
														echo '<a target="_blank" href="'.asset("storage").'/business/proof_of_due/'.$img_name.'">View</a>|<a  href="'.asset("storage").'/business/proof_of_due/'.$img_name.'" download>Download</a>';
                                                    }
													?>
                                    ...
                                    <td>
										<a href="" class="btn btn-primary paymentHistoryButton" data-toggle="modal" data-target="#paymentHistory" data-due-id="{{$data->dueId}}" data-profile-id="{{$data->profile->id}}" data-custom-id="{{$data->external_business_id}}" title="Payment History">
	  ...
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  ...
		  <div class="modal-footer">
		  	<button type="reset" class="btn btn-primary btn-blue" data-dismiss="modal">close</button>
		  </div>
		</div>
	  </div>
	</div>
           ...
           <?php }?>
        	  var businessId = $(this).data('profile-id');
          		var customId = $(this).data('custom-id');
					$.ajax({
					   method: 'post',
					   url: "{{route('user-business-payment-history-for-admin')}}",
					   headers: {
