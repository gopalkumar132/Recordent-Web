...
<form action="{{route('edit-profile-store',$userId)}}" method="POST" id="edit_profile_form" enctype="multipart/form-data">
...
<div class="panel-body">
				...
				<?php 
				$emailReadonly = "";
				if($user_profile->email_verified_at!="" || $user_profile->email_sent_at!="") {
            ...
            <div class="col-md-8">
                ...
                <div class="form-group form-group-default col-md-12" id="emailGroup">
                  <div class="col-md-2">Email*</div>
                         ...
                         <input type="text" name="email" id="email" value="{{ old('email',$user_profile->email) }}" placeholder="E-mail" class="form-control email" {{$emailReadonly}} autocomplete="off" onkeyup='saveValue(this);' onBlur='sendVerifyEmail(this);' onkeypress='sendVerifyEmailPress(this);' maxlength="{{General::maxlength('email')}}">
							...
							<?php if($user_profile->email!="" && $user_profile->email_verified_at!="") { ?>
							<i class="fa fa-check fa-2x" style="color:green;" aria-hidden="true"></i>
							<?php } ?>
							</div>
							<input type="hidden" id="emailexists" value="{{$user_profile->email}}">
							<input type="hidden" id="emailverified" value="{{$user_profile->email_verified_at}}">
							
                </div>
				<div class="form-group form-group-default col-md-12">
                            <div class="col-md-2">Mobile</div>
							<div class="controls col-md-8">
                                <input type="text" name="mobile_number" id="mobile_number" value="{{ old('mobile_number',$user_profile->mobile_number) }}" readonly placeholder="Mobile" class="form-control" maxlength="10">
                            </div>
							<div class="col-md-2">
							<?php if($user_profile->mobile_number!="" && $user_profile->mobile_verified_at!="") { ?>
							<i class="fa fa-check fa-2x" style="color:green;" aria-hidden="true"></i>
							<?php } else { ?>
                                 ...
                                 <select class="form-control" id="company_id" name="company_id" required >
									...
									@if(count($company_types))
								            @foreach($company_types as $ckey=>$ctype)
								            	<option value="{{$ckey}}" {{old('company_id',$user_profile->company_type)==$ckey ? 'selected' : '' }}>{{$ctype}}</option>
								            @endforeach  
							            @endif
                                </select>
								</div>
				...
				<div class="form-group form-group-default col-md-12">
							...
							<div class="controls company-type-class" id="display__companyid_err">
                                <input type="text" name="gstin" id="gstin" value="{{ old('gstin',$user_profile->gstin_udise) }}" placeholder="E-mail" maxlength="15" class="form-control company_type_dynamic" onkeyup='removeCustomError(this);'>
				...
				<div class="form-group form-group-default col-md-12">
							...
							<div class="controls col-md-10">
                                <input type="text" name="business_name" id="business_name" value="{{ old('business_name',$user_profile->business_name) }}" {{$gstinVerifyReadonly}} placeholder="Business/Organisation Name" class="form-control" maxlength="{{General::maxlength('name')}}">
                            </div>
                </div>
				<div class="form-group form-group-default col-md-12" id="emailGroup">
                            <div class="col-md-2">Business Short Name</div>
							<div class="controls col-md-10">
                                <input type="text" name="business_short" id="business_short" value="{{ old('business_short',$user_profile->business_short) }}" placeholder="Business Short Name" class="form-control" maxlength="50">
                            </div>
                </div>
				...
				<div class="form-group form-group-default col-md-12 others-type">
									...
									<select name="sector_id" id="sector_id"  placeholder="Select Sector" class="form-control">
							            ...
							            @if($userTypes->count())  
								            @foreach($userTypes as $usertype)
								            	<option value="{{$usertype->id}}" {{old('sector_id',$user_profile->user_type)==$usertype->id ? 'selected' : '' }}>{{$usertype->name}}</option>
								            @endforeach  
							            @endif
							        </select>
									</div>
					        </div>
							<input type="hidden" id="business_type_id_hidden" value="{{$user_profile->user_type}}">
                        ...
                        <div class="form-group form-group-default col-md-12" id="type_of_business_div" style="display:none">
                            ...
                            <div class="controls col-md-10">
                                <input type="text" name="type_of_business" id="type_of_business" value="{{ old('type_of_business',$user_profile->type_of_business) }}" placeholder="Please specify type of business" class="form-control">
                            </div>
				...
				<div class="form-group form-group-default col-md-12">
							...
							<div class="controls col-md-10">
                                <input type="text" name="address" id="address" value="{{ old('address',$user_profile->address) }}" {{$gstinVerifyReadonly}} placeholder="Address" class="form-control" maxlength="100">
                            </div>
                </div>
                        <div class="form-group form-group-default col-md-12" id="emailGroup" >
                            <div class="col-md-2">State*</div>
                            <div class="controls col-md-10">
							<?php $stateCheck = $user_profile->state!=""? $user_profile->state->id : "";?>
				...
				<div class="form-group form-group-default col-md-12">
							...
							<div class="controls col-md-10">
                                <input type="text" name="pincode" id="pincode" value="{{ old('pincode',$user_profile->pincode) }}" {{$gstinVerifyReadonly}} placeholder="Pincode" class="form-control" maxlength="6">
                            </div>
                </div>
				</div>
        </div>
        ...
        <div class="col-md-4">
            <div class="panel panel-bordered">
                ...
                <img src="{{ filter_var($user_profile->avatar, FILTER_VALIDATE_URL) ? $user_profile->avatar : Voyager::image( $user_profile->avatar ) }}" style="width:200px; height:auto; clear:both; display:block; padding:2px; border:1px solid #ddd; margin-bottom:10px;" />
                @endif
               ...
               success:function(gstinData) {
				...
				$("#gstin_error").html(respoonseData.data);   
				   ...
				   if(gstinCharAt == "P" || gstinCharAt == "p") {
					   $("#business_name").val(result.tradeNam);
				   } else {
					   $("#business_name").val(result.lgnm);
				   }
