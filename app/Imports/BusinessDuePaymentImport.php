...
class BusinessDuePaymentImport implements ToModel, WithHeadingRow, WithEvents
{
...
{   
	...
	$rule = [
	   ...
	   'payment_amount' => 'required|numeric|gt:0|lte:1000000000',
	...
	if(!$this->hasIssue){
    	$dueData = BusinessDueFees::where('invoice_no','LIKE',$row['invoice_no']);
    	if($row['customer_id']!=''){
    		$dueData = $dueData->where('business_id',$row['customer_id']);
    ...
    if($this->hasIssue){
		...
		$paid = BusinessPaidFees::create([
			...
			'created_at'=>$currentDate,
			'external_business_id'=>$dueData->external_business_id,
			'added_by'=>$authId,
		]);
		return $paid;
	}
